...
description: 前端工程开发规范技能。用于 `apps/frontend` 相关任务，包括 TanStack Router 路由拆分与重定向、Alova 请求封装与 ApiEnvelope 解包、页面解耦分层（Route/Page/Component/Hook/Service/Context）、前端错误码映射与状态管理一致性、项目级注释规范，以及类型检查/lint/验收清单。当需求涉及新增或重构前端路由、调整接口调用层、统一错误处理、补充前端验收规则、全局状态共享（Context）设计、补充注释规范、或评审前端实现一致性时使用。
---
...
- 注释遵循仓库顶层注释规范：中文默认，专有名词可英文，且仅强制新增/修改代码。
