...
- 本文档基于 [UI 需求文档（v1.1）](./ui-requirements.md) 的决策背景，不重述主需求主体内容。
- 本文档不替代 [产品需求文档](./requirements.md)，也不修改其主体内容。
- 继承约束说明：本扩展仅定义“历史搜索”增量；登录态前置、统一响应结构（ApiEnvelope）、路由 query 基础规则统一继承 `ui-requirements.md v1.1`。
...
9. 文档一致性验收：本扩展文档不得与 `ui-requirements.md v1.1` 中“当前契约/目标契约”分层定义冲突。
...
- 默认不修改 `ui-requirements.md`，仅修订本扩展文档。
